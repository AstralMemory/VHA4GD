...
func _init():
	if !FileAccess.file_exists("user://config"):
		var f = FileAccess.open("user://config", FileAccess.WRITE)
		var config_data = {
			"access_token": "",
			"refresh_token": "",
			"character_id": "",
...
func rewrite_config(mode: String = "rewrite", key: String = "", value: String = "", hide_message = 0):
	match mode:
		"rewrite":
			if FileAccess.file_exists("user://config"):
				var config_file = FileAccess.open("user://config", FileAccess.READ)
				if config_file.get_length():
					var config_data = bytes_to_var(config_file.get_buffer(config_file.get_length()))
					config_file.close()
					DirAccess.remove_absolute("user://config")
					var new_config = FileAccess.open("user://config", FileAccess.WRITE)
